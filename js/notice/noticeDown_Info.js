	...
	$scope.showStationTree=function(){
	   ...
	   //执行查询语句
	    ...
	    var ModalInstanceCtrl = function ($scope, $modalInstance,$http,ngDialog) {
	    	...
	    	$http({
      	   		 method:'POST',
      				 url:'getSelectStationTreeByUser.do',
      				 params:'',
      				 headers: { 'Content-Type': 'application/json; charset=UTF-8'}
      				 })
      				 .success(function(response){
  				    	 console.log(response);
  				    	 $scope.m.stationTree = response;
      	   	     }); 	
	    	...
	    	$scope.save = function(){
	    		...
	    		angular.forEach($scope.m.stationids, function ( item ) {
	    			ids = ids +","+item.id;
	    	    });
	    		
	    		
	    		var param = $scope.m;
	    		param.stationIds = ids;
	    		console.log(ids);
	    		if(mtype==1){
	    			$http({
	       	   		 method:'POST',
	       				 url:'saveNoticeDown.do',
	       				 params:param,
	       				 headers: { 'Content-Type': 'application/json; charset=UTF-8'}
	       				 })
	       				 .success(function(response){
