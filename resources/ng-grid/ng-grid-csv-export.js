        ...
        function showDs() {
        	...
        	for (var f in grid.config.columnDefs) {
                if (grid.config.columnDefs.hasOwnProperty(f)) {
                	values.push(grid.config.columnDefs[f].displayName);
                }
            }
            var keys = [];
            for (var f in grid.config.columnDefs) {
            ...
            var gridData = scope.$parent.$parent.csvData;
            ...
            csvDataLinkHtml += "<div style=\"margin-top:10px;margin-left:150px\"><a href=\"data:text/csv;charset=UTF-8,";
            csvDataLinkHtml += encodeURIComponent(csvData);
            csvDataLinkHtml += "\" download=\""+opts+".csv\" style=\"color:black\">导出EXCEL</a></div></span>";
